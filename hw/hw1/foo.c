...
int main(){
	...
	printf("-----poping from stack2 and pushig to stack3-----\n");
	for(i=0; i<5; i++){

	int* t2 = 0;
	t2 = (int*)pop(stack2);			
	push(stack3, t2);
	printf("add data in stack 3 : %d\n",*t2);

	} // pop stack 2 -> push stack 3
	printf("stack1 count is : %d\n", stack1 -> count);
	printf("stack2 count is : %d\n", stack2 -> count);
	printf("stack3 count is : %d\n", stack3 -> count);
	
	printf("\n");
	printf("----poping from stack3 and pushig to stack1-----\n");

	printf("------- 0,1,2,3,4 and..------\n");
